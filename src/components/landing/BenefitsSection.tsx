...
const features = [
  ...
  },
  {
    ...
    icon: MessagesSquare,
    className: "md:col-start-3 md:col-span-1 md:row-start-1 md:row-span-1.75",
  },
  {
    ...
    icon: TrendingUp,
    className: "md:col-start-3 md:col-span-1 md:row-start-2.75 md:row-span-2",
  },
];

const BentoGrid = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="grid w-full grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 md:auto-rows-[160px]">
      {children}
    </div>
  );
};
...
const BentoCard = ({
  ...
  <motion.div
    ...
    className={cn(
      "group relative overflow-hidden rounded-[20px] p-4 lg:p-5 h-full",
      "bg-white",
  ...
  >
    <div className="relative z-10 h-full flex flex-col justify-between">
      <div className="flex-1 flex flex-col justify-end mb-2.5">
        <div className="mb-2.5 rounded-full w-10 h-10 lg:w-11 lg:h-11 flex items-center justify-center
                      bg-gradient-to-br from-zinc-50 to-white
                      shadow-[0_2px_4px_rgba(16,24,40,0.08)]
                      group-hover:scale-110 transition-transform duration-300">
          <Icon className="w-5 h-5 lg:w-5.5 lg:h-5.5 text-zinc-900" />
        </div>
        <h3 className="font-display text-base lg:text-lg text-zinc-900 tracking-tight">
          {title}
        </h3>
      </div>
